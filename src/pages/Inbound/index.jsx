...
export default function Inbound() {
    ...
    useEffect(() => {
        ...
        .then(res => {
            if (Array.isArray(res.data.data)) {
                ...
                setError({ message: 'Data yang diterima tidak valid.' });
            }
        })
        .catch(err => {
    ...
    const deleteInbound = (id) => {
        ...
        .catch(err => {
            setError({ message: 'Gagal menghapus barang.' });
        });
    };
                ...
                <div className="m-5 pb-10 pt-10">
                    ...
                    <div className="flex mt-4 md:mt-6">
                        <table className="min-w-full text-left text-sm font-light">
                            ...
                            <tbody>
                                {Array.isArray(inbound) && inbound.map((item, index) => (
                                    <tr key={item.id} className="border-b dark:border-neutral-500">
                                        <td className="text-white px-6 py-4">{index + 1}</td>
                                        <td className="text-white px-6 py-4">{item.stuff_id}</td>
                                        <td className="text-white px-6 py-4">{item.total}</td>
                                        <td className="text-white px-6 py-4">{item.date}</td>
                                        <td className="text-white px-6 py-4">
                                            <a href={"http://localhost:8000/upload-images//"+item.proff_file} alt ="poster.jpeg" target={"http://localhost:8000/upload-images/"+item.proff_file}>
                                            <img src={UrlImage + item.proff_file} alt="ikieww.jpeg" className="w-16 h-auto object-cover" />
