    ...
    const restoreStuff = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.get(`http://localhost:8000/stuff/restore/${id}`, null, config)
                    .then(() => {   
            ...
            <div className="block w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <div className="m-5 pb-10 pt-10">
                    <div className="flex justify-between">
                        <h5 className="mb-1 ml-5 text-3xl font-medium text-white dark:text-white">Stuff Trash</h5>
                        <div className="flex">
                    ...
                    {error && (
                        <div role="alert">
                            <div className="bg-red-500 text-red font-bold rounded-t px-4 py-2">
                                Gagal!
                            </div>
                            <div className="border border-t-0 border-red-400 rounded-b bg-red-100 px-4 py-3 text-red">
                                {error}
                            </div>
                        </div>
                    )}
                    <div className="overflow-x-auto">
                        <table className="w-full table-auto">
                            ...
                            <tbody>
                                {trash.map(stuff => (
                                    <tr key={stuff.id}>
                                        <td className="border px-4 py-2 text-white">{stuff.name}</td>
                                        <td className="border px-4 py-2 text-white">{stuff.category}</td>
